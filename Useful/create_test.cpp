...
void Manual(const string& filename) {
	...
	cout << "\t+) Task folder to generate must be in workspace folder (for example: Workspace/TASK/)\n";
#if _WIN32
...
int main(int argc, char** argv) {
	...
	try {
		...
		for (auto const& entry : fs::directory_iterator{original_workspace}) {
...
#endif
				cout << color(red) << "[-] Solution binary file (solution.exe) not found! Skipped task\n";
...
#endif
				cout << color(red) << "[-] Generator binary file (generator.exe) not found! Skipped task\n";
				skipped.push_back(task_name);
				continue;
			}
			Green();
			...
			for (int test = 1; test <= test_cases; ++test) {
				...
				execute(workspace_wins + "\\\\generator.exe > " + folder + "/" + task_name + ".INP");
				cout << "[ ] Generating output...\n";
				execute(workspace_wins + "\\\\solution.exe < " + folder + "/" + task_name + ".INP" + " > " + folder + "/" + task_name + ".OUT");
#else
				execute(workspace + "/generator > " + folder + "/" + task_name + ".INP");
				cout << "[ ] Generating output...\n";
				execute(workspace + "/solution < " + folder + "/" + task_name + ".INP" + " > " + folder + "/" + task_name + ".OUT");
#endif // LOCAL
			}
	...
	cout << "\nBatch completed in " << (double) clock() / CLOCKS_PER_SEC << " seconds.\n";
